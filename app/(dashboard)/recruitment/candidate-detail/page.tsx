...
export default function CandidateDetailPage() {
  ...
  const handleHireEmployee = () => {
    // Store candidate data for pre-filling the employee form
    ...
    <div className="p-4 md:p-6 xl:p-8 bg-gray-50 min-h-screen overflow-x-hidden">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-6 mb-6">
        <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4 mb-6">
          <div className="flex items-start gap-4">
            ...
            <div>
              <h1 className="text-xl font-bold text-[#153453] mb-2">{candidate.name}</h1>
          ...
          <span className={`px-4 py-2 rounded-full text-sm font-semibold border-2 ${getStatusColor(candidate.status)}`}>
            {candidate.status}
          </span>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-[#dbe7f1] rounded-xl p-4">
            <p className="text-xs font-medium text-gray-600 mb-1">Candidate ID</p>
            <p className="text-sm font-bold text-[#1e3147]">{candidate.id}</p>
          </div>
          ...
          <div className="bg-[#dbe7f1] rounded-xl p-4">
            ...
            <p className="text-sm font-bold text-[#1e3147]">{candidate.experience}</p>
          </div>
          <div className="bg-[#e7f0f5] rounded-xl p-4">
            <p className="text-xs font-medium text-gray-600 mb-1">Applications</p>
            <p className="text-sm font-bold text-[#2c4a6a]">{candidate.applications}</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-6">
            ...
            <div className="flex flex-wrap gap-2">
              {candidate.skills.map((skill, idx) => (
                <span key={idx} className="px-3 py-2 bg-[#d4e1ed] text-[#2c4a6a] rounded-lg text-xs font-medium border border-[#a8c5db]">
                  {skill}
                </span>
              ))}
            </div>
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs font-semibold text-gray-900">{candidate.email}</p>
                </div>
              </div>
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs font-semibold text-gray-900">{candidate.phone}</p>
                </div>
              </div>
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-full bg-[#d4e1ed] flex items-center justify-center flex-shrink-0">
                ...
                <div>
                  ...
                  <p className="text-xs font-semibold text-gray-900">{candidate.location}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          {candidate.applicationHistory && candidate.applicationHistory.length > 0 && (
            <div className="bg-white rounded-2xl border border-gray-100 p-6">
              <div className="flex items-center gap-3 mb-4">
                ...
                <h2 className="text-base font-bold text-gray-900">Application History</h2>
              </div>
              <div className="space-y-3">
                {candidate.applicationHistory.map((app, idx) => (
                  <div key={idx} className="p-4 bg-gray-50 rounded-lg border border-gray-100">
                    <p className="text-xs font-semibold text-gray-900">{app.position}</p>
                    <p className="text-xs text-gray-600 mt-1">{app.date} • {app.status}</p>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="space-y-6">
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-600 mt-0.5">{new Date(candidate.registeredDate).toLocaleDateString("en-GB", { day: "numeric", month: "long", year: "numeric" })}</p>
                </div>
              </div>
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-full bg-[#e8eef4] flex items-center justify-center flex-shrink-0">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-600 mt-0.5">{candidate.status}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          {/* Candidate Stats */}
          <div className="bg-white rounded-2xl border border-gray-100 p-6">
            <h2 className="text-base font-bold text-gray-900 mb-5">Candidate Stats</h2>
            <div className="space-y-4">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-xs text-gray-500 mb-0.5">Total Applications</p>
                  <p className="text-xs font-semibold text-gray-900">{candidate.applications}</p>
                </div>
              </div>
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-xs text-gray-500 mb-0.5">Experience Level</p>
                  <p className="text-xs font-semibold text-gray-900">{candidate.experience}</p>
                </div>
              </div>
              <div className="flex items-start gap-3">
                <div className="w-9 h-9 rounded-lg bg-[#dbe7f1] flex items-center justify-center flex-shrink-0">
