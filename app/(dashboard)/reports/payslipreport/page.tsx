...
export default function PayslipReportPage() {
  ...
  React.useEffect(() => {
    ...
    if (filterPaymentStatus !== "All") result = result.filter(p => filterPaymentStatus === "Paid" ? p.isPaid : !p.isPaid);
  ...
  const totalNetPay = filteredPayslips.reduce((s, p) => s + p.netPay, 0);
    ...
    <div className="p-4 md:p-6 xl:p-8 bg-gray-50 min-h-screen overflow-x-hidden">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        {[
          ...
          { label: "Paid",            value: paidCount,                               sub: `${filteredPayslips.length ? ((paidCount / filteredPayslips.length) * 100).toFixed(1) : 0}% completed` },
          ...
          <div key={c.label} className="bg-gradient-to-br from-[#2c4a6a] to-[#1e3147] rounded-xl p-5">
            ...
            <p className="text-3xl font-bold text-white">{typeof c.value === "number" ? c.value.toLocaleString() : c.value}</p>
              ...
              <tbody className="divide-y divide-gray-100">
                {currentPayslips.map(payslip => (
                  <tr key={payslip.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4"><p className="text-sm text-gray-900">{payslip.position}</p></td>
      ...
      {filteredPayslips.length === 0 && (
        <div className="bg-white rounded-2xl border border-gray-100 p-12 text-center">
          ...
          <button onClick={() => { setSearchTerm(""); setFilterDepartment("All"); setFilterPaymentStatus("All"); }}
      ...
      {filteredPayslips.length > 0 && (
        <div className="bg-white rounded-2xl border border-gray-100 p-4 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-1.5">
            ...
            {/* FIX: ellipsis uses index-based key, page numbers use prefixed string key */}
            <div className="hidden sm:flex gap-1">
              {pageNums().map((p, i) =>
                p === "..."
                  ? <span key={`ellipsis-${i}`} className="w-9 flex items-center justify-center text-gray-400 text-sm">…</span>
                  : <button key={`page-${p}`} onClick={() => goTo(p as number)}
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="bg-[#2c4a6a]/5 rounded-xl p-5 border border-[#2c4a6a]/10">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                  <div><p className="text-gray-500 text-xs mb-0.5">Name</p><p className="font-semibold text-gray-900">{selectedPayslip.name}</p></div>
                  <div><p className="text-gray-500 text-xs mb-0.5">Employee ID</p><p className="font-semibold text-gray-900">{selectedPayslip.employeeId}</p></div>
                  <div><p className="text-gray-500 text-xs mb-0.5">Department</p><p className="font-semibold text-gray-900">{selectedPayslip.department}</p></div>
                  <div><p className="text-gray-500 text-xs mb-0.5">Position</p><p className="font-semibold text-gray-900">{selectedPayslip.position}</p></div>
                </div>
              </div>
              ...
              <div className="bg-[#2c4a6a]/5 rounded-xl p-5 border border-[#2c4a6a]/10">
                ...
                <div className="space-y-2">
                  <div className="flex justify-between py-2 border-b border-gray-100"><span className="text-sm text-gray-600">Basic Salary</span><span className="text-sm font-semibold">₵{selectedPayslip.basicSalary.toLocaleString()}</span></div>
                  <div className="flex justify-between py-2 border-b border-gray-100"><span className="text-sm text-gray-600">Allowances</span><span className="text-sm font-semibold">₵{selectedPayslip.allowances.toLocaleString()}</span></div>
                  <div className="flex justify-between py-2 border-b border-gray-100 bg-[#eef3f9] px-3 rounded"><span className="text-sm font-medium">Gross Pay</span><span className="text-base font-bold text-[#2c4a6a]">₵{selectedPayslip.grossPay.toLocaleString()}</span></div>
                  <div className="flex justify-between py-2 border-b border-gray-100"><span className="text-sm text-gray-600">Tax (10%)</span><span className="text-sm font-semibold text-gray-700">- ₵{selectedPayslip.taxAmount.toLocaleString()}</span></div>
                  <div className="flex justify-between py-2 border-b border-gray-100"><span className="text-sm text-gray-600">SSNIT (5.5%)</span><span className="text-sm font-semibold text-gray-700">- ₵{selectedPayslip.ssnitAmount.toLocaleString()}</span></div>
                  <div className="flex justify-between py-2 border-b border-gray-100"><span className="text-sm text-gray-600">Other deductions</span><span className="text-sm font-semibold text-gray-700">- ₵{(selectedPayslip.deductions - selectedPayslip.taxAmount - selectedPayslip.ssnitAmount).toLocaleString()}</span></div>
                  <div className="flex justify-between py-3 bg-[#2c4a6a]/5 px-3 rounded"><span className="text-base font-bold text-[#1e3147]">Net Pay</span><span className="text-xl font-bold text-[#2c4a6a]">₵{selectedPayslip.netPay.toLocaleString()}</span></div>
                </div>
              </div>
              ...
              <div className="bg-[#2c4a6a]/5 rounded-xl p-5 border border-[#2c4a6a]/10">
                    ...
                    <tbody className="divide-y divide-gray-200">
                      {selectedPayslip.salaryHistory.map((h, i) => (
                        <tr key={i} className="hover:bg-gray-50">
                          ...
                          <td className="px-4 py-3 text-sm text-right text-gray-600">₵{h.deductions.toLocaleString()}</td>
                          <td className="px-4 py-3 text-sm text-right font-semibold text-[#2c4a6a]">₵{h.netPay.toLocaleString()}</td>
                          <td className="px-4 py-3 text-center">
                            <span className="px-2 py-1 text-xs rounded-full bg-[#d4e1ed] text-[#2c4a6a] border border-[#a8c5db]">Paid</span>
                          </td>
                        </tr>
                      ))}
                    </tbody>
                ...
                <div className="grid grid-cols-3 gap-4 mt-4 pt-4 border-t border-gray-200">
                  <div className="text-center"><p className="text-xs text-gray-500 mb-1">Avg Gross</p><p className="text-sm font-bold text-gray-900">₵{Math.round(selectedPayslip.salaryHistory.reduce((s, h) => s + h.grossPay, 0) / 6).toLocaleString()}</p></div>
                  <div className="text-center"><p className="text-xs text-gray-500 mb-1">Avg Deductions</p><p className="text-sm font-bold text-gray-700">₵{Math.round(selectedPayslip.salaryHistory.reduce((s, h) => s + h.deductions, 0) / 6).toLocaleString()}</p></div>
                  <div className="text-center"><p className="text-xs text-gray-500 mb-1">Avg Net</p><p className="text-sm font-bold text-[#2c4a6a]">₵{Math.round(selectedPayslip.salaryHistory.reduce((s, h) => s + h.netPay, 0) / 6).toLocaleString()}</p></div>
                </div>
              </div>
            </div>

            <div className="flex gap-3 justify-end p-6 border-t">
              <button onClick={() => setIsDetailModalOpen(false)} className="px-6 py-2.5 border border-gray-200 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors">Close</button>
